...
import { MailCheck, Clock, AlertCircle } from "lucide-react";

export default function SentEmailsTable({ refreshTrigger }: { refreshTrigger: number }) {
  ...
  if (emails.length === 0) {
    return (
      <div className="text-center py-12 border-2 border-dashed border-gray-200 rounded-lg">
        ...
        <p className="mt-1 text-sm text-gray-500">Emails will appear here once they are dispatched.</p>
      </div>
    );
  }
            ...
            <tr key={email.id} className="hover:bg-gray-50 transition-colors">
              ...
              <td className="px-6 py-4 whitespace-nowrap text-sm">
                ...
                {email.status === 'FAILED' && email.failureReason && (
                  <div className="text-xs text-red-500 mt-1 max-w-xs truncate" title={email.failureReason}>
                    {email.failureReason}
                  </div>
                )}
              </td>
